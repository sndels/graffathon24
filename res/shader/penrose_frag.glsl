...
bool tail(in vec2 p, int tId, out int tIds[REC_DEPTH], out vec2 uvs[REC_DEPTH], out int types[REC_DEPTH])
{
...
bool fname(in vec2 p, int tId, out int tIds[REC_DEPTH], out vec2 uvs[REC_DEPTH], out int types[REC_DEPTH])\
{                                                                          \
    vec2 uv = triangles.data[tId].uvmInv * (p-triangles.data[tId].o);           \
    if (uv.x >= 0.0 && uv.y >= 0.0 && uv.x + uv.y < 1.0) {                 \
        for (int i=0; i<3; ++i) {                                          \
            ...
            if (tId2 == 0)                                                 \
                break;                                                     \
            if (fchildname(p, tId2, tIds, uvs, types)) break;                      \
        }                                                          \
        uvs[reclevel] = uv; \
        tIds[reclevel] = tId; \
    ...
    for (int tId=0; tId<10; ++tId) {
        if (recf1(p, tId, tIds, uvs, types)) {
            ...
            if (dColorStyle < 0.5) {
                ...
                vec3 w1 = vec3(square(1.0-z1), 2.0*z1*(1.0-z1), z1*z1);
                float z2 = clamp(zoomFrac * 0.5, 0.0, 1.0);
                vec3 w2 = vec3(square(1.0-z2), 2.0*z2*(1.0-z2), z2*z2);
